 ...
 * @Date: 2022/8/13 - 08 - 13 - 23:24
 * @Description: com.mashibing.tank.Bullet
 * @version: 1.0
 */
public class RedBulletFactory extends BulletFactory {
    public static RedBulletFactory bulletFactory;
    static {
        bulletFactory=new RedBulletFactory();
    }
    //将构造方法隐藏
    private RedBulletFactory(){
    }
    //获取子弹工厂的引用
    ...
    public  List<Bullet> getBulletsFourDir(Tank tank){
        ...
        for (int i = 0; i < bullets.size(); i++) {
            ((RedBullet)bullets.get(i)).setCoordinate(tank);
        }
        return bullets;
    }
    //获取全屏子弹
    ...
    public  List<Bullet> getBulletsAll(Tank tank){
        ...
        for (int i = 0; i < quantitys.length; i++) {
            ...
            for (int j = 0; j < quantitys[i]; j++) {
                switch (dirs[i]){
                    case UP:
                        coordinates[i][j] = new Coordinate((TankFrame.GAME_WIDTH-RedBullet.WIDTH) / quantitys[i]*j+((TankFrame.GAME_WIDTH-RedBullet.WIDTH) / quantitys[i])/2, TankFrame.GAME_HEIGHT - RedBullet.HEIGHT);
                        break;
                    case DOWN:
                        coordinates[i][j] = new Coordinate((TankFrame.GAME_WIDTH-RedBullet.WIDTH) / quantitys[i]*j+((TankFrame.GAME_WIDTH-RedBullet.WIDTH) / quantitys[i])/2, 20);
                        break;
                    case LEFT:
                        coordinates[i][j] = new Coordinate(TankFrame.GAME_WIDTH-RedBullet.WIDTH, (TankFrame.GAME_HEIGHT-RedBullet.WIDTH) / quantitys[i]*j+((TankFrame.GAME_HEIGHT-RedBullet.HEIGHT) / quantitys[i])/2);
                        break;
                    case RIGHT:
                        coordinates[i][j] = new Coordinate(20, (TankFrame.GAME_HEIGHT-RedBullet.HEIGHT) / quantitys[i]*j+((TankFrame.GAME_HEIGHT-RedBullet.HEIGHT) / quantitys[i])/2);
                        break;
                }
            }
        }
    ...
    public  List<Bullet> getBullets(Tank tank,BulletDemand demand){
        ...
        for (int i = 0; i < demand.size; i++) {
            ...
            for (int j = 0; j < demand.quantity[i]; j++) {
                RedBullet bullet=new RedBullet(demand.coordinates[i][j],tank.camp,demand.dirs[i]);
                //速度
...
//普通子弹
    public  Bullet getBullet(Tank tank){
        RedBullet bullet=new RedBullet(tank.coordinate,tank.camp,tank.dir);
